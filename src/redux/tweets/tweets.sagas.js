...
export function* updateTweet({payload: {id, tag, message, email}}) {
    try {
        const tweet = yield axios.put("http://34.224.86.128:8080/api/v1.0/tweets/update", {
            id: id,
            tags: tag,
            message: message,
            email: email
...
export function* deleteTweet({payload: {id, email}}) {
    try {
        const tweet = yield axios.delete("http://34.224.86.128:8080/api/v1.0/tweets/delete",
            {
                params: {
                    username: email,
                    tweetId: id
...
export function* postTweet({payload: {id, tag, message, email}}) {
    try {
        const tweet = yield axios.post("http://34.224.86.128:8080/api/v1.0/tweets/add/", 
        {
            id: id,
            tags: tag,
            message: message,
...
export function* getAllTweets() {
    try {
        const tweets = yield axios.get("http://34.224.86.128:8080/api/v1.0/tweets/all", {
            params: {
                page: 0,
                size: 10
            }
...
export function* getMyTweets({payload: {email}}) {
    try {
        const tweets = yield axios.get("http://34.224.86.128:8080/api/v1.0/tweets/username", {
            params: {
                email: email 
            }
        });
